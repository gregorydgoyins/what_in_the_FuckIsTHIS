...
import { TrendingDown, Info, AlertTriangle, Calendar, DollarSign } from 'lucide-react';

export function PutsPage() {
  const [selectedSymbol, setSelectedSymbol] = useState('ASM300');
  const [selectedExpiry, setSelectedExpiry] = useState('2024-09-20');
  
  // Mock put options data
  const mockPutOptions = [
    {
      strike: 2400,
      premium: 120,
      volume: 275,
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <h1 className="text-3xl font-bold text-white">Put Options</h1>
        </div>
      </div>
      ...
      <div className="bg-slate-800/90 backdrop-blur-md rounded-xl p-6 shadow-xl">
        <div className="flex items-start space-x-4 mb-6">
          ...
          <p className="text-gray-300">
            Put options give the holder the right, but not the obligation, to sell the underlying asset at a specified price (strike price) before a certain date (expiration date). Puts are often used as insurance against price declines or to profit from bearish price movements.
          </p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-slate-700/50">
            ...
            <tbody className="divide-y divide-slate-700/50">
              {mockPutOptions.map((option, index) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
          <div className="bg-slate-700/50 p-4 rounded-lg border border-slate-600/50">
            <h3 className="font-medium text-white mb-3">Put Option Basics</h3>
            <ul className="space-y-2 text-sm text-gray-300">
              <li className="flex items-start space-x-2">
                <DollarSign className="h-4 w-4 text-green-400 flex-shrink-0 mt-0.5" />
                <span>Put buyers profit when the price of the underlying asset falls below the strike price minus the premium paid.</span>
              </li>
              <li className="flex items-start space-x-2">
              ...
              <li className="flex items-start space-x-2">
                ...
                <span>Put sellers (writers) have the obligation to buy the underlying asset if the option is exercised.</span>
              </li>
              <li className="flex items-start space-x-2">
                <Info className="h-4 w-4 text-indigo-400 flex-shrink-0 mt-0.5" />
                <span>In-the-money puts have strike prices above the current market price.</span>
              </li>
            </ul>
          </div>
