...
export function CreatorStock({ 
  ...
  const [selectedRole, setSelectedRole] = useState(filter?.role || 'All');
  const [selectedRating, setSelectedRating] = useState(filter?.rating || 'All');
  const [sortBy, setSortBy] = useState('marketCap');
  const [selectedCard, setSelectedCard] = useState<string | null>(null);

  const mockCreatorStocks: CreatorStock[] = [
    {
      id: '1',
      name: 'Todd McFarlane',
      symbol: 'TMFS',
  ...
  const filteredCreators = mockCreatorStocks
    .filter(creator => {
  ...
  return (
    <div className="bg-slate-800/90 backdrop-blur-md rounded-xl p-6 shadow-xl">
      ...
      <div className={compact ? "space-y-4" : "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6"}>
        {filteredCreators.map((creator) => (
          <div
            key={creator.id}
            onClick={() => setSelectedCard(selectedCard === creator.id ? null : creator.id)}
            className={`group relative bg-slate-700/50 border border-slate-600/50 rounded-lg p-4 hover:shadow-[0_0_15px_rgba(255,255,0,0.7)] transition-all hover:-translate-y-1 cursor-pointer touch-target
      ...
      {!compact && filteredCreators.length > 0 && filteredCreators.length < mockCreatorStocks.length && (
